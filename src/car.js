...
export class Hypercar {
  constructor(scene) {
    ...
    // Build the chosen procedural hypercar immediately (no GLTF delay, no street car)
    this.buildCurrentHypercar();
    this.isReady = true;
    this.readyCallbacks = [];
  }
  ...
  buildCurrentHypercar() {
    while (this.carModelWrapper.children.length > 0) {
      this.carModelWrapper.remove(this.carModelWrapper.children[0]);
    }

    const cfg = CAR_CONFIGS[this.carIndex] || CAR_CONFIGS[0];
    const built = buildProceduralHypercar(cfg);

    this.activeCarModel = built.group;
    this.carModelWrapper.add(this.activeCarModel);
...
export class RemoteHypercar {
  ...
  updateTelemetry(data) {
    ...
    if (data.steerAngle !== undefined && this.steerFL && this.steerFR) {
      this.steerFL.rotation.y = data.steerAngle;
      this.steerFR.rotation.y = data.steerAngle;
    }
  ...
  update(dt) {
    ...
    this.wheelRollAngle += (spd / 0.35) * dt;
    if (this.wheelFL) this.wheelFL.rotation.x = this.wheelRollAngle;
    if (this.wheelFR) this.wheelFR.rotation.x = this.wheelRollAngle;
    if (this.wheelRL) this.wheelRL.rotation.x = this.wheelRollAngle;
    if (this.wheelRR) this.wheelRR.rotation.x = this.wheelRollAngle;
